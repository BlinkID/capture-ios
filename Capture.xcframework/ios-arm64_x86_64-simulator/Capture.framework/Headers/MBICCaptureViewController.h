...
#import <Capture/MBICAnalyserResult.h>
#import <Capture/MBICCaptureSettings.h>
...
///
/// Returns on the `analyserQueue`. Dispatch every UI change on the main queue.
///
/// - Parameters:
///    - captureViewController: Capture document view controller.
///    - analyserResult: Frame analyser result. See `MBICAnalyserResult` for available results.
- (void)captureViewController:(nonnull MBICCaptureViewController *)captureViewController didFinishCaptureWithResult:(nonnull MBICAnalyserResult *)analyserResult NS_SWIFT_NAME(captureViewController(captureViewController:didFinishCaptureWithResult:));
